...
def main():
	...
	test = pd.read_csv(test_file, converters={"timestamp":parse})
	sol = pd.read_csv(solution_file)
	...
	test_sol = pd.merge(test, sol, how='outer', left_on=['user','event'], right_on=['User','Events'])
	test_user = pd.merge(test_sol, users, left_on='user', right_on='user_id')
	tue = pd.merge(test_user, events, left_on='event', right_on='event_id')
	...
	for row_index, row in tue_pop.iterrows():
		...
		if not math.isnan(row['User']):
			tue_pop['class'][row_index] = 1
	...
	select.to_csv('test_feature.csv', index=False)
